#!/bin/sh
# start next line using wish\
exec wish "$0" "$@"

#
# BufGet.tcl  Version 4:  Modsnd2002 Bufkit and archive path support
#
#############################

#########################################################################
#  Set Your Three Options here:
#
...
set PASSIVE 0
#
# 2.  Do you have Internet assess through a proxie server?
#      1  means TRUE
#      0  means FALSE
#    Most sites do not go through a Proxie server to gain Internet access.
#    In this case set PROXIE to No (i.e. 0) and ou can ignore
#    the PROXIESERVER setting.
#    However, if you have to go through a proxie server,
#    set PROXIE to TRUE (i.e. 1) AND set PROXIESERVER to the name
#    (or IP Number) of the proxie server,
#
set PROXIE 0
set PROXIESERVER  192.168.0.1
##############################

proc BuildGUI {} {
    global tcl_platform
    
    # tk_setPalette burlywood2
    
    
    #   wm geometry . 700x500
    wm title . "BufGet 4"
    
    label  .lURL         -text "Profile List A:"
    button .bGetProfiles -text "Get Profiles"    -width 10   -command {GetProfiles}
    
    frame .fe
    button .fe.bCopy   -text Copy   -width 4 -command Copy
    button .fe.bPaste  -text Paste  -width 4 -command Paste
    button .fe.bBufkit -text Bufkit -width 4 -command RunBufkit
    pack .fe.bCopy .fe.bPaste .fe.bBufkit -side left
    
    button .bExit        -text "Exit"            -width 9    -command {exit}
    
    grid .lURL          -row 0 -column 0  -sticky sw
    grid .bGetProfiles  -row 0 -column 2  -pady 6
    grid .fe            -row 0 -column 3
    grid .bExit         -row 0 -column 5   -sticky e
    
    text  .tURL -fg green   -bg black  -font {Clean 13 bold} -insertbackground white \
            -height 20  -width 80  -relief sunken
    if  {$tcl_platform(platform) == "windows"} {
        .tURL configure -font "System"
    }
    
    grid .tURL -columnspan 6
    
    frame .fLoad -relief groove -borderwidth 2
    frame .fSave -relief groove -borderwidth 2
    label .fLoad.lLoad -text "Load Profile List"
    label .fSave.lSave -text "Save Profile List"
    pack  .fLoad.lLoad
    pack  .fSave.lSave
    foreach letter {A B C D E F G} {
        button .fLoad.bLoad$letter -text $letter -pady 0          -command [list Load $letter]
        pack   .fLoad.bLoad$letter -side left
        button .fSave.bSave$letter -text $letter -pady 0 -bg red  -command [list Save $letter]
        pack   .fSave.bSave$letter -side left
    }
    grid .fLoad -row 2 -column 0 -columnspan 3  -pady 6
    grid .fSave -row 2 -column 3 -columnspan 3
    
    label .lStatus -text Status:
    grid .lStatus -sticky sw
    
    text  .tStatus -font {"MS Sans Serif" 9} -height 4.5   -relief sunken
    grid  .tStatus -sticky nsew -columnspan 6
}

proc Load {letter} {
    global argv0
    if [catch {open [file join [file dirname $argv0] "BufGet$letter.Dat"] r} fid] {
        .tStatus insert insert "\nProfile List $letter Not Yet Created"
        .tStatus see insert
    } else {
        .tURL delete 1.0 end
        .tURL insert insert [read $fid]
        close $fid
        .lURL config -text "Profile List $letter"
        .tStatus insert insert "\nProfile List $letter Loaded"
        .tStatus see insert
    }
}

proc Save {letter} {
    set fid [open "BufGet$letter.Dat" w]
    puts -nonewline $fid [.tURL get 1.0 end]
    close $fid
    .tStatus insert insert "\nProfile List $letter Saved"
    .tStatus see insert
}

proc Copy {} {
    clipboard clear
    catch {clipboard append [selection get -displayof .tURL] }
}

proc Paste {} {
    catch {.tURL insert insert [selection get -selection CLIPBOARD]}
}

proc GetDirectories {} {
    global Path ArchivePath argv0
    
    set readfrom [file join [file dirname $argv0] "wse.map"]
    if [catch {open [file join [file dirname $argv0] "Bufkit.cfg"] r} fid] {
        tk_messageBox -type ok \
                -message "Could Not Locate the Bufkit Configuration File: BUFKIT.CFG\n\nMake Sure BUFGET.TCL is installed in your BUFKIT Directory." \
                -icon warning\
                -title "Woops!  Installation Check!"
        exit
    } else {
        gets $fid buffer
        gets $fid Path
        gets $fid buffer
        gets $fid ArchivePath
        close $fid
    }
}


proc FTPProgress {total} {
    global Size
    set Size $total
    .tStatus insert insert "*"
}

proc FTPdata {URL path} {
    global Size
    global PROXIE PASSIVE
    set Size 0
    set p [string first "://" $URL]
    set buffer [string range $URL [expr $p+3] 9999]
    set p [string first "/" $buffer]
    set site [string range $buffer 0 [expr $p-1]]
    set plast [string last "/" $buffer]
    set directory [string range $buffer $p [expr $plast-1]]
    set profile [string range $buffer [expr $plast+1] 9999]
    set SaveAs [file join $path $profile]
    
    #  FTP::Open $site "anonymous" "bufkitnoaa.gov" -progress FTPProgress -mode passive
    #  FTP::Open "192.168.0.1" "anonymous@$site" "bufkit@noaa.gov" -progress FTPProgress
    
    if {$PROXIE} {
        global PROXIESERVER
        FTP::Open $PROXIESERVER "anonymous@$site" "bufget@noaa.gov" -progress FTPProgress
    } elseif {$PASSIVE} {
        FTP::Open $site "anonymous" "bufget@noaa.gov" -progress FTPProgress -mode passive
    } else {
        FTP::Open $site "anonymous" "bufget@noaa.gov" -progress FTPProgress -mode passive
    }
    ...
    FTP::Cd $directory
    FTP::Type binary
    FTP::Get $profile $SaveAs
    FTP::Close
    
    if {$Size < 10000} {
        .tStatus insert insert "....ERROR - Profile Not Found ($Size) - Check the URL"
        file delete $SaveAs
    } else {
        .tStatus insert insert "FTP OK ($Size)"
    }
}

proc HTTPProgress {token totalsize currentsize} {
    global Size
    set Size $currentsize
    .tStatus insert insert "*"
}

proc HTTPdata {URL path} {
    global Size PROXIE
    set Size 0
    set p [string last "/" $URL]
    set profile [string range $URL [expr $p+1] 9999]
    set SaveAs [file join $path $profile]
    set out [open $SaveAs w]
    
    if {$PROXIE} {
        global PROXIESERVER
        http::config -proxyhost $PROXIESERVER -proxyport 80
    }
    if {[catch {http::geturl $URL -progress HTTPProgress -channel $out} token]} {
        .tStatus insert insert "....ERROR - Site Not Found....($Size)"
    } else {
        .tStatus insert insert "HTTP [string range [http::code $token] 12 9999] ($Size)"
    }
    close $out
    if {$Size < 1000} {
        file delete $SaveAs
...
proc GetProfiles {} {
    global Path ArchivePath
    .tStatus delete 1.0 end
    for {set line 1} { $line < 9999 } {incr line} {
        set URL [string trim [.tURL get $line.0 $line.end]]
        if {$URL != ""} {
            .tStatus insert insert \n$URL
            .tStatus see end
            update idletasks
            
            if {[regexp {.+/[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]\.+} $URL] == 1} {
                set dir $ArchivePath
                puts "1>using Archive Path >$ArchivePath<"
            } else  {   
                set dir $Path
                puts "2>using Path >$Path<"
            }
            
            
            if {[string match "http://*" [string tolower $URL]]} {
                HTTPdata $URL $dir
            } elseif {[string match "ftp://*" [string tolower $URL]]} {
                FTPdata $URL $dir
            } else {
                .tStatus insert insert "....ERROR! Bad Protocol In URL....Skipped"
                .tStatus see end
                continue
            }
        } else {
...
proc RunBufkit {} {
    ...
    if  {$tcl_platform(platform) == "windows"} {
        exec [file join [file dirname $argv0] "Bufkit_AWWT_Simulations.exe"] &
    } else  {
        exec wine [file join [file dirname $argv0] "Bufkit_AWWT_Simultions.exe"] &
    }
}
