...
function get_my_url (bounds) {
  ...
  var geojsoni = new OpenLayers.Layer.GML("Global Bufkit Sites", 
    "global_network.php?"+ (new Date()).getTime(),
            {
   ...
   function get_my_url (bounds) {
    ...
    var hpc = new OpenLayers.Layer.Vector("HPC 5-Day Precip", {
    ...
    hpc.setVisibility(false);

    var spcdy1 = new OpenLayers.Layer.Vector("SPC Day 1 Outlook", {
        projection: map.displayProjection,
        strategies: [new OpenLayers.Strategy.Fixed()],
        protocol: new OpenLayers.Protocol.HTTP({
            ...
            format: new OpenLayers.Format.KML({
                ...
                extractAttributes: true
		//extractTracks: true,
		//trackAttributes: ["Categorical"]
            })
        }),
    });

    spcdy1.setVisibility(false);

    var wdssii_ref = new OpenLayers.Layer.Vector("NSSL Radar", {
        projection: map.displayProjection,
        strategies: [new OpenLayers.Strategy.Fixed()],
        protocol: new OpenLayers.Protocol.HTTP({
            ...
            format: new OpenLayers.Format.KML({
                extractStyles: true,
                extractAttributes: true
		//extractAttributes: ["Categorical"]
            })
        }),
    });

    //wdssii_ref.setVisibility(false);
   ...
   warnings.setVisibility(false);
        ...
        var gstreets = new OpenLayers.Layer.TMS("Google Streets", "https://mt1.google.com/vt/lyrs=h@132&hl=en&s=sphericalMercator",
                { type:'png',
  ...
  map.addLayers([osm,geojsoni,irsat,vissat,n0q,hpc,spcdy1,warnings]);
