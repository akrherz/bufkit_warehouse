...
#  $myFile = "//lan/bufkit_data/"."$myModel"."_"."$myStation".".buf";
  $myFile = "/home/ckarsten/WWW/bufkit/data/nam/"."$myModel"."_"."$myStation".".buf";
  open(BUFIN,$myFile) or goto GETSTATION ; 
    if     ($myModel eq "eta" or $myModel eq "etam" or $myModel eq "etaw" or $myModel eq "nam" or $myModel eq "nmm" or $myModel eq "arw") { ReadEtaBufkit() }
...
sub GetLyrMeans {
  ...
  $lyrLapse[$i][0] = ($tmpc[$i][$j] - $tmpc[$i][$j+1])/$lyrThick[$i][0] * 1000.;
