		...
		if(is_int($int)){
			...
			elseif($model == "gfs" || $model == "gfsm"){
				$hght[$z][] = $d[0];
                        ...
                        $sknt[$z][] = $d[6];
                        $omeg[$z][] = $d[7];
			// max temp in profile & momentum transfer
			if($k == 1){
				$maxt[$z] = $d[1];
				$t_last = $d[1];
				$p_last = $d[0];
			...
			elseif($d[1] > $t_last){
				$maxt[$z] = $d[1];
				$t_layer_diff = ($d[1] + $t_last) / 2;
				$dt = $d[1] - $t_last;
				$dz = (($r * $t_layer_diff)/$g) * log($p_last / $d[0]);
				$t_last = $d[1];
				$p_last = $d[0];				
			}
		}		
