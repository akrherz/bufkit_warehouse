...
# v5.1 - Nice improvements for sleet, and new format output
# v5.2 - Makes the web display better with additional headers every x amount of lines
# V5.3 - Makes RUC available.
# v5.4 - Changes the layer thickness calc to be more accurate.  
#
#
#######################################################################################################
...
#  $myFile = "//lan/bufkit_data/"."$myModel"."_"."$myStation".".buf";
  $myFile = "/home/ckarsten/WWW/bufkit/data/"."$myModel"."_"."$myStation".".buf";
  ...
  $cumHours = 0; $cumPrecipHours = 0; $cumSnow = 0.0; $cumSleet = 0.0; $cumIce = 0.0; $cumPrecip = 0.0; $cumSR = 0.0; 
  if ( $statime eq "" ) {
    $= = ($#fcstHR + 4) ;
    for($i = 1; $i <= $#fcstHR; $i++) { 
       ...
       if ( $cumPrecip > 0.0 ) {$cumSR = $cumSnow / $cumPrecip }
       if ( $precip[$i] < 0.006 ) { if ( $myModel ne "gfs3") {$cumPrecipHours += 1.0 } else { $cumPrecipHours += 3.0 }} else { $cumPrecipHours = 0.0 } 
	   if ( $cumPrecipHours > 6.0 ) {$cumSnow = 0.0 ; $cumPrecip = 0.0 ; $cumSR = 0.0 ; $cumSleet = 0.0 ; $cumIce = 0.0 ; $cumPrecipHours = 0.0 ; }
       if ( $myModel ne "gfs3") {$cumHours += 1.0 } else { $cumHours += 3.0 }
        if ( $cumHours > 6 and $myModel ne "gfs3" ) {
          $linecount = $linecount + 1;
          if ( $linecount > 5) {
            $linecount = 0;
            $cumHours = 1;
            print "-------------------------------------------------------------------------------------------------------------------\n";
            print "  Date/hour    FHr  Wind    SfcT   Ptype  SRat|Snow||TotSN    QPF ||TotQPF   Sleet||TotPL    FZRA||TotZR    S%| I%| L%\n";
            print "-------------------------------------------------------------------------------------------------------------------\n"
          } else {
            $cumHours = 1 ;
            #print $linecount ;
        ...
        } elsif ( $cumHours > 12 and $myModel eq "gfs3" ) {
           $linecount = $linecount + 1;
           if ( $linecount > 7) {
            $linecount = 0;
            $cumHours = 1;
...
#  if ($statime eq "")  { STDOUT->format_name("VER50"); STDOUT->format_top_name("VER50_TOP")}
  if ($statime eq "")  { STDOUT->format_name("VER51"); STDOUT->format_top_name("VER51_TOP")}
...
format DIAG_TOP =
  Station: @<<<  DiagTime: F+@||HR  Profile Thermal Adjust: @#.#  Cloud RH threshold: @##%
           $myStation            $statime                    $thermalShift               $RHThresh
                                                                    ...
                                                                    Warm  Cold1 Cold2 
   Pres    T(C)| Tw  |  Td      LR |RHw|RHi|  Uvv      Lsr| Wtvv     >0C | <0C | <-6C 
==================================================================================================================
.
format DIAG = 
@####MB   @##.#|@##.#|@##.#  @###.#|@##|@##|@##.#    @##:1|@#.##    @##.#|@##.#|@##.#
$lyrPres[$i][$j] $lyrTemp[$i][$j] $lyrTmwc[$i][$j] $lyrDwpc[$i][$j] $lyrLapse[$i][$j] $lyrRH[$i][$j] $lyrRHice[$i][$j] $lyrOmega[$i][$j] $layerSR[$j] $percentArea[$i][$j] $lyrPosEnergy[$i][$j] $lyrNegEnergy1[$i][$j] $lyrNegEnergy2[$i][$j]
.
...
.
format VER51_TOP =
StnID: @<<<   Model: @<<<   Run: 20@<<<<<<<<<<    Cloud RH threshold: @##%    Sleet Ratio: 2:1   || CarSnowTool Beta 5.4
...
sub GetLyrMeans {
  ...
  my $j;
  for ($j = $#{$pres[$i]}; $j >=1; $j--) { 
     $lyrRH[$i][$j] = ($rh[$i][$j] + $rh[$i][$j-1])/2.0;
     $lyrRHice[$i][$j] = ($rhi[$i][$j] + $rhi[$i][$j-1])/2.0;
     $lyrDwpc[$i][$j] = ($dwpc[$i][$j] + $dwpc[$i][$j-1])/ 2.0;
     $lyrTmwc[$i][$j] = ($tmwc[$i][$j] + $tmwc[$i][$j-1])/ 2.0;
     $lyrTemp[$i][$j] = ($tmpc[$i][$j] + $tmpc[$i][$j-1])/ 2.0 ;
     $lyrPres[$i][$j] = ($pres[$i][$j] + $pres[$i][$j-1])/ 2.0 ;
	 if ($myModel eq "gfs3" and ($j-1) == 0 ) { 
        $lyrThick[$i][$j] = -2.0}
	  else {
        $lyrThick[$i][$j] = $hght[$i][$j] - $hght[$i][$j-1]}
     $lyrOmega[$i][$j] = (($omeg[$i][$j] + $omeg[$i][$j-1])/ 2.0);
### Do not use avgHour = YES ... problems with calcs here...
	 if ( $avgHour eq "YES" and $i > 1 ) {
        $lyrRH[$i][$j] = ($rh[$i][$j] + $rh[$i-1][$j] + $rh[$i][$j-1] + $rh[$i-1][$j-1])/4.0;
        $lyrRHice[$i][$j] = ($rhi[$i][$j] + $rhi[$i-1][$j] + $rhi[$i][$j-1] + $rhi[$i-1][$j-1])/4.0;
		$lyrDwpc[$i][$j] = ($dwpc[$i][$j] + $dwpc[$i][$j-1] + $dwpc[$i-1][$j] + $dwpc[$i-1][$j-1])/4.0;
        $lyrTmwc[$i][$j] = ($tmwc[$i][$j] + $tmwc[$i][$j-1] + $tmwc[$i-1][$j] + $tmwc[$i-1][$j-1])/4.0;
        $lyrTemp[$i][$j] = ($tmpc[$i][$j] + $tmpc[$i][$j-1] + $tmpc[$i-1][$j] + $tmpc[$i-1][$j-1])/4.0;
        $lyrPres[$i][$j] = ($pres[$i][$j] + $pres[$i][$j-1] + $pres[$i-1][$j] + $pres[$i-1][$j-1])/4.0;
        $lyrOmega[$i][$j] = ($omeg[$i][$j] + $omeg[$i][$j-1] + $omeg[$i-1][$j] + $omeg[$i-1][$j-1])/4.0;
...
#print "I am in GetSnow :-)  $i \n" ;
  for($j = $#{$pres[$i]}; $j >=1; $j--) {
    if ( $lyrRHice[$i][$j] >= $RHThresh  and  $lyrOmega[$i][$j] < 0.0 ) {           ### Need to fix lyrRH thresh...
      ...
      if ( $lyrTemp[$i][$j] < -28.0 ) {
         $tdiff = 0.0;
         ($a, $b, $c, $d) = split(/ /,"10.00 0.0000 0.0000 0.0000") }
       elsif ( $lyrTemp[$i][$j] < -21.0 ) {
         $tdiff = $lyrTemp[$i][$j] + 28.0;
         ($a, $b, $c, $d) = split(/ /,"10.00 0.2874 0.0000 0.0204") }
       elsif ( $lyrTemp[$i][$j] < -18.0 ) {
         $tdiff = $lyrTemp[$i][$j] + 21.0;
         ($a, $b, $c, $d) = split(/ /,"19.00 3.2824 0.4279 -0.0999") }
       elsif ( $lyrTemp[$i][$j] < -15.0 ) {
         $tdiff = $lyrTemp[$i][$j] + 18.0;
         ($a, $b, $c, $d) = split(/ /,"30.00 3.1517 -0.4714 -0.0079") }
       elsif ( $lyrTemp[$i][$j] < -12.0 ) {
         $tdiff = $lyrTemp[$i][$j] + 15.0;
         ($a, $b, $c, $d) = split(/ /,"35.00 0.1107 -0.5423 -0.0908") }
       elsif ( $lyrTemp[$i][$j] < -10.0 ) {
         $tdiff = $lyrTemp[$i][$j] + 12.0;
         ($a, $b, $c, $d) = split(/ /,"28.00 -5.5946 -1.3595 0.4534") }
       elsif ( $lyrTemp[$i][$j] < -8.0 ) {
         $tdiff = $lyrTemp[$i][$j] + 10.0;
         ($a, $b, $c, $d) = split(/ /,"15.00 -5.5917 1.3609 -0.1575") }
       elsif ( $lyrTemp[$i][$j] < -5.0 ) {
         $tdiff = $lyrTemp[$i][$j] + 8.0;
         ($a, $b, $c, $d) = split(/ /,"8.00 -2.0384 0.4157 -0.0232") }
       elsif ( $lyrTemp[$i][$j] < -3.0 ) {
         $tdiff = $lyrTemp[$i][$j] + 5.0;
         ($a, $b, $c, $d) = split(/ /,"5.00 -0.1705 0.2069 -0.0608") }
       elsif ( $lyrTemp[$i][$j] <= 0.0 ) {
         $tdiff = $lyrTemp[$i][$j] + 3.0;
         ($a, $b, $c, $d) = split(/ /,"5.00 -0.0729 -0.1581 0.0105") }
       elsif ( $lyrTemp[$i][$j] > 0.0 ) {
         $tdiff = 0.0;
