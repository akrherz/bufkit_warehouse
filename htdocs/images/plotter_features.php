			...
			$h++;
				...
				elseif($var1 == "wind"){
					...
					$snow[] = $d[$index];
						...
						if($ratio != 0){
	                                                $ruc_var[] = array_sum($snow);
						}
                                                $ruc_var1[] = array_sum($snow1);
                                        }
                                }
                                elseif($var1 == "wind"){
                                        $ruc_var[] = $d[$index];
                                        if($d[$index1] == 0){
                                                ...
                                                $ruc_var1[] = $d[$index1] * 1.15077945; 
                                        }
                                        if($d[$index2] == 0){
                                                $ruc_var2[] = $d[$index]; 
                                        }
                                        else{
                                                $ruc_var2[] = $d[$index2] * 1.15077945; 
                                        }
                                }
				else{
	                                $ruc_var[] = $d[$index];
			...
			if(@$d[11] == "Z"){
				...
				if($z == 0){
					if($h == 0){
						...
						$nam_var2[] = 0;
						$nam_cobb_init = date('H',$nam_cobb_time[0]);
					}
						...
						$nam_var2[] = array_sum($cobb_snow);
                                ...
                                elseif($z == 1){
                                        if($h == 0){
                                                ...
                                                $namm_var2[] = 0;
                                                $namm_cobb_init = date('H',$namm_cobb_time[0]);
                                        }
                                ...
                                elseif($z == 2){
                                        if($h == 0){
                                                ...
                                                $gfs_var2[] = 0;
                                                $gfs_cobb_init = date('H',$gfs_cobb_time[0]);
                                        }
                                ...
                                elseif($z == 3){
                                        if($h == 0){
                                                ...
                                                $gfsm_var2[] = 0;
                                                $gfsm_cobb_init = date('H',$gfsm_cobb_time[0]);
                                        }
...
//$min = min($mins);
//$max = max($maxs);
if(!empty($buf_t_gfs) && !empty($buf_t_gfsm)){
...
if($obs == 1 && in_array($site,$sites)){
	...
	foreach($data as $line){
		...
		if($ob_time >= $min && $d[2] != -99 && $k >= 0 && $minute >= 51 && $minute <= 56){
			$obs_lat = $d[10];
			$obs_lon = $d[9];
...
if($nws == 1 && in_array($site, $sites)){                          
        ...
        foreach($data as $line){        
			...
			if($var == "qpf_accum" || $var1 == "snow_accum"){
	...
	if($var1 == "wind"){
		$link4_5 = "&product=time-series&begin=".$init_time."%3A00%3A00&end=".$end_time."%3A00%3A00&Unit=e&wgust=wgust&Submit=Submit";
	        $link4 = "".$link4_1."".$link4_2."".$link4_3."".$link4_4."".$link4_5."";
		$data = file($link4);
	        foreach($data as $line){
	                preg_match_all(".$nws_t.", $line, $id);
...
if($con == 1){
	for($i=$min;$i<=$max;$i=$i+10800){
		...
		if(in_array($i,$buf_t_nam) && !empty($nam_var) && $nam == 1){
        	...
        	if(in_array($i,$buf_t_ruc) && !empty($ruc_var) && $ruc == 1){
        	        $n++;
		...
		if(in_array($i,$gfs_mos_time) && !empty($gfs_mos_var) && $gfs_mos == 1){
                ...
                if(in_array($i,$buf_t_ruc) && !empty($ruc_var1) && $ruc == 1){
                        $n1++;
                        $index = array_search($i,$buf_t_ruc);
                        $total1 = $total1 + $ruc_var1[$index];
                }
		...
		if($var1 == "snow_accum"){
	                ...
	                if(in_array($i,$buf_t_ruc) && !empty($ruc_var2) && $ruc == 1){
	                        $n2++;
	                        $index = array_search($i,$buf_t_ruc);
	                        $total2 = $total2 + $ruc_var2[$index];
	                }
		}
...
if($ruc == 1 && !empty($ruc_var) && !empty($buf_t_ruc) && count($ruc_var) == count($buf_t_ruc)){
	$lineplot_ruc=new LinePlot($ruc_var,$buf_t_ruc);
	$lineplot_ruc->SetColor("green");
	$lineplot_ruc->SetWeight(3);
	if($var1 == "snow_accum"){
		if($ratio != 0){
                        $lineplot_ruc->SetLegend("".$buf_ruc_init."z RUC ".$ratio.":1");
                        $graph->Add($lineplot_ruc);
                }
        }
        elseif($var1 == "wind"){             
                if($mean == 1){ 
                        $lineplot_ruc->SetLegend("".$buf_ruc_init."z RUC");             
                        $graph->Add($lineplot_ruc);
                }    
        }
        else{
		$lineplot_ruc->SetLegend("".strval($buf_ruc_init)."z RUC");
		$graph->Add($lineplot_ruc);
	}
}

if($ruc == 1 && !empty($ruc_var1) && !empty($buf_t_ruc) && count($ruc_var1) == count($buf_t_ruc)){
        $lineplot_ruc1=new LinePlot($ruc_var1,$buf_t_ruc);
        $lineplot_ruc1->SetColor("green");
        $lineplot_ruc1->SetWeight(3);
        $lineplot_ruc1->SetStyle("dashed");
        if($var1 == "snow_accum"){
		if($max_t == 1){
	                $lineplot_ruc1->SetLegend("".$buf_ruc_init."z RUC Max-T Prof");
        	        $graph->Add($lineplot_ruc1);
        	}
	}
        elseif($var1 == "wind"){
		if($mean_mt == 1){
	                $lineplot_ruc1->SetLegend("".$buf_ruc_init."z RUC Mean MT");
        	        $graph->Add($lineplot_ruc1);
        	}
	}
        else{
                $lineplot_ruc1->SetLegend("".$buf_ruc_init."z RUC");
                $graph->Add($lineplot_ruc1);
        }
}

if($ruc == 1 && !empty($ruc_var2) && !empty($buf_t_ruc) && count($ruc_var2) == count($buf_t_ruc)){
        $lineplot_ruc2=new LinePlot($ruc_var2,$buf_t_ruc);
        $lineplot_ruc2->SetColor("darkgreen");
        $lineplot_ruc2->SetWeight(3);
        $lineplot_ruc2->SetStyle("dashed");      
        if($var1 == "wind"){
                $lineplot_ruc2->SetLegend("".$buf_ruc_init."z RUC Max MT");
		if($max_mt == 1){
                	$graph->Add($lineplot_ruc2);
		}
        }
        else{
                $lineplot_ruc2->SetLegend("".$buf_ruc_init."z RUC");
                $graph->Add($lineplot_ruc2);
        }
}
