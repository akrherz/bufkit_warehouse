...
for($z=0;$z<=4;$z++){
	...
	$link = "http://www.meteor.iastate.edu/~ckarsten/bufkit/data/snow.php?model=".$mdl."&site=".$site_l."&ratio=".$ratio."";
	//echo $link;
	//die();
	$temp_maxt = 0;
	$temp_sr = 0;
...
$link4_5 = "&product=time-series&begin=".$init_time."%3A00%3A00&end=".$end_time."%3A00%3A00&Unit=e&temp=temp&qpf=qpf&snow=snow&dew=dew&wspd=wspd&wdir=wdir&Submit=Submit";
...
foreach ($data as $line) {
     ...
     preg_match_all(".$s_v5.", $line, $id10);
     $check10 = @$id10[0][0];

     preg_match_all(".$s_v6.", $line, $id11);
     $check11 = @$id11[0][0];

     preg_match_all(".$s_v7.", $line, $id12);
     $check12 = @$id12[0][0];

     preg_match_all(".$s_v8.", $line, $id13);
     $check13 = @$id13[0][0];

     preg_match_all(".$s_v9.", $line, $id14);
     $check14 = @$id14[0][0];
     ...
     if($check1 == $s_v || $check7 == $s_v2 || $check8 == $s_v3 || $check9 == $s_v4 || $check10 == $s_v5 || $check11 == $s_v6 || $check12 == $s_v7 || $check13 == $s_v8 || $check14 == $s_v9){
          $trip = $trip+1;
     }
